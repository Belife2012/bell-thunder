...
public:
    ...
    void setup(WireType& w = Wire)
    {
        ...
        if (m_whoami)
        {
            ...
            {
                Serial.printf("Could not connect to AK8963: 0x%x\n", AK8963_WHOAMI_DEFAULT_VALUE);
            }
        }
        else
        {
            Serial.printf("Could not connect to MPU9250: 0x%x\n", MPU9250_WHOAMI_DEFAULT_VALUE);
        }
    }
    ...
    // TODO: more efficient getter, const 保证函数不对成员变量进行修改
