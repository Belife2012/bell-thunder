...
void MOTOR_SERVO::Servo_Turn_Percent(int servo, float percent, float speed)
{
	...
	if (percent > 0)
	{
		percent = servo_percent_zero[servo] + (servo_percent_max[servo] - servo_percent_zero[servo]) * percent / 100;
	}
	else
	{
		percent = servo_percent_zero[servo] + (servo_percent_zero[servo] - servo_percent_min[servo]) * percent / 100;
	}

	last_position = servo_position[servo];
	servo_position[servo] = Servo_MIN + Servo_Range * (percent + 100) / 200;
	if(speed <= 99){
		if(last_position < servo_position[servo]) {
			while(last_position < servo_position[servo]) {
				if (servo == 1) //A口
				{
		...
		} else if(last_position > servo_position[servo]) {
			while(last_position > servo_position[servo]) {
				if (servo == 1) //A口
				{
					ledcWrite(SERVO_CHANNEL_0, last_position);
	...
	if (servo == 1) //A口
	{
		ledcWrite(SERVO_CHANNEL_0, servo_position[servo]);
	}
	else if (servo == 2) //B口
	{
		ledcWrite(SERVO_CHANNEL_1, servo_position[servo]);
	}
}
